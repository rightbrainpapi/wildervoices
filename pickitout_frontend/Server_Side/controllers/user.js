...
//-------------Signup In Functionality-------------------
  ...
  // },
    ...
    getuserbio: (req,res)=>{
      if(req.params.id){ //<-- if there is a req.userId with the (verified._id) saved in it then it runs the function
        db.User.findById(req.params.id, (err, foundUser)=>{  //<-- send back the founduser to the frontend (which is App.js)
